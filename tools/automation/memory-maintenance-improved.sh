...
verify_environment() {
    ...
    if [[ ! -d "$WORKSPACE" ]]; then
        echo "❌ Critical error: Workspace directory not found: $WORKSPACE" >&2
        echo "   Try: export OPENCLAW_WORKSPACE=/path/to/your/workspace" >&2
        ((errors++))
    fi

    if [[ ! -d "$MEMORY_DIR" ]]; then
        echo "❌ Critical error: Memory directory not found: $MEMORY_DIR" >&2
        echo "   Expected: $WORKSPACE/memory" >&2
        ((errors++))
    fi
    
    if [[ ! -w "$LOG_DIR" ]]; then
        echo "❌ Critical error: Cannot write to log directory: $LOG_DIR" >&2
        ((errors++))
    fi
    
    if [[ $errors -gt 0 ]]; then
        echo "❌ Environment verification failed. Cannot continue." >&2
        exit 1
    fi
}
...
safe_append_log() {
    ...
    # Atomic append operation
    {
        ...
        rm -f "$temp_file"
        echo "❌ Failed to write log entry" >&2
        return 1
    }
}
...
analyze_memory_growth() {
    ...
    if ! daily_files=$(find "$MEMORY_DIR" -name "2026-*.md" -type f 2>/dev/null); then
        echo "   ⚠️  Could not access daily memory files" | tee -a "$LOG_FILE"
        return 1
    fi
    ...
    while IFS= read -r file; do
        ...
        if ! [[ -r "$file" ]]; then
            echo "   ⚠️  Cannot read file: $(basename "$file")" | tee -a "$LOG_FILE"
            continue
        fi
        ...
        if word_count=$(wc -w < "$file" 2>/dev/null); then
            if (( word_count > 500 )); then
                echo "   📈 Large daily file: $(basename "$file") ($word_count words)" | tee -a "$LOG_FILE"
                found_large=true
            fi
        else
            echo "   ⚠️  Could not analyze: $(basename "$file")" | tee -a "$LOG_FILE"
        fi
    done <<< "$daily_files"
...
check_cross_references() {
    ...
    if [[ ! -f "$index_file" ]]; then
        echo "   ⚠️  Cross-reference index not found" | tee -a "$LOG_FILE"
        echo "   💡 Consider creating index for improved navigation" | tee -a "$LOG_FILE"
        return 1
    fi
    ...
    if index_date=$(stat -c %Y "$index_file" 2>/dev/null); then
        if files_since=$(find "$MEMORY_DIR" -name "*.md" -newer "$index_file" -type f 2>/dev/null | wc -l); then
            if (( files_since > 0 )); then
                echo "   📝 $files_since memory files created since last index update" | tee -a "$LOG_FILE"
                if (( files_since > 5 )); then
                    echo "   💡 Consider updating cross-reference index" | tee -a "$LOG_FILE"
                fi
            else
                echo "   ✅ Cross-reference index is current" | tee -a "$LOG_FILE"
            fi
        else
            echo "   ⚠️  Could not check index currency" | tee -a "$LOG_FILE"
        fi
    else
        echo "   ⚠️  Could not read index file statistics" | tee -a "$LOG_FILE"
    fi
}
...
analyze_consolidation_opportunities() {
    ...
    if ! temp_analysis=$(mktemp); then
        echo "   ⚠️  Could not create temporary analysis file" | tee -a "$LOG_FILE"
        return 1
    fi
    
    # Safe content analysis with error handling
    {
        echo "📈 Topic distribution analysis:"
        ...
        if (( openclaw_count > 10 )); then
            echo "💡 Consider consolidating OpenClaw content (spread across $openclaw_count files)"
        fi
        
    } | tee -a "$LOG_FILE" 2>/dev/null || {
        echo "   ⚠️  Content analysis failed" | tee -a "$LOG_FILE"
    }
    ...
    if total_files=$(find "$MEMORY_DIR" -name "*.md" -type f 2>/dev/null | wc -l); then
        ...
        if total_size=$(du -sb "$MEMORY_DIR" 2>/dev/null | cut -f1); then
            ...
            if (( total_files > 50 )); then
                echo "   💡 Large file count may slow search - consider archiving old content" | tee -a "$LOG_FILE"
            elif (( total_size > 10000000 )); then  # 10MB
                echo "   💡 Large memory size - monitor search performance" | tee -a "$LOG_FILE"  
            else
                echo "   ✅ Memory system size optimal for performance" | tee -a "$LOG_FILE"
            fi
        else
            echo "   ⚠️  Could not calculate memory system size" | tee -a "$LOG_FILE"
        fi
    else
        echo "   ⚠️  Could not count memory files" | tee -a "$LOG_FILE"
    fi
}
    ...
    if last_optimization=$(find "$MEMORY_DIR" -name "*OPTIMIZATION*" -printf "%T@ %p\n" 2>/dev/null | sort -n | tail -1 | cut -d' ' -f1 | cut -d'.' -f1); then
        ...
        if [[ -n "$last_optimization" && "$last_optimization" -gt 0 ]]; then
            ...
            if (( days_since > 14 )); then
                echo "   🔧 Consider comprehensive memory system review ($days_since days since last optimization)" | tee -a "$LOG_FILE"
            fi
        fi
    fi
...
main() {
    echo "🧠 Memory System Maintenance - $(date)" | tee "$LOG_FILE"
    echo "================================================" | tee -a "$LOG_FILE"
