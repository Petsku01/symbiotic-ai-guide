...
acquire_lock() {
    ...
    while ! mkdir "$lock_file" 2>/dev/null; do
        if (( wait_time >= timeout )); then
            echo "❌ Failed to acquire lock: $lock_file (timeout after ${timeout}s)" >&2
            return 1
        fi
        ...
        if [[ -d "$lock_file" ]]; then
            ...
            if lock_age=$(( $(date +%s) - $(stat -c %Y "$lock_file" 2>/dev/null || echo 0) )); then
                if (( lock_age > 600 )); then  # 10 minutes
                    echo "⚠️  Removing stale lock (${lock_age}s old): $lock_file" >&2
...
release_lock() {
    ...
    if [[ -d "$lock_file" ]]; then
        ...
        if rmdir "$lock_file" 2>/dev/null; then
            echo "✅ Lock released: $lock_file" >&2
        else
            echo "⚠️  Failed to release lock: $lock_file" >&2
            return 1
        fi
    fi
    return 0
...
create_secure_log() {
    ...
    if ! chmod 600 "$log_file"; then
        echo "❌ Failed to set secure permissions on: $log_file" >&2
        return 1
    fi
...
atomic_write() {
    ...
    if ! echo "$content" > "$temp_file"; then
        rm -f "$temp_file"
        echo "❌ Failed to write temporary file: $temp_file" >&2
        return 1
    fi
    ...
    if ! mv "$temp_file" "$target_file"; then
        rm -f "$temp_file"
        echo "❌ Failed to move temporary file to target: $target_file" >&2
        return 1
    fi
...
atomic_log_append() {
    ...
    temp_file=$(mktemp "${log_file}.append.XXXXXX") || {
        echo "❌ Failed to create temporary file for log append" >&2
        return 1
    }
    ...
    # Atomic append operation
    {
        ...
        rm -f "$temp_file"
        echo "❌ Failed to append to log: $log_file" >&2
        return 1
    }
...
validate_config() {
    ...
    if [[ ! -d "$workspace" ]]; then
        ...
        echo "❌ Workspace directory not readable: $workspace" >&2
        ((errors++))
    elif [[ ! -w "$workspace" ]]; then
        echo "❌ Workspace directory not writable: $workspace" >&2
        ((errors++))
    fi
