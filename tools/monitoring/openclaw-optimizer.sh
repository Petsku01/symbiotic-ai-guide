...
echo "🔧 OpenClaw Performance Optimization Recommendations"
echo "===================================================="
echo ""
...
if [ -z "$OPENCLAW_PID" ]; then
    echo "❌ OpenClaw gateway not running - cannot analyze"
    exit 1
fi
...
if [ $MEMORY_MB -gt 1500 ]; then
    echo "🔴 HIGH PRIORITY - Memory usage over 1.5GB:"
...
if [ $DISCONNECTS -gt 5 ]; then
    echo "🔴 HIGH PRIORITY - Frequent network disconnections:"
...
if [ $SLOW_EVENTS -gt 3 ]; then
    echo "🟡 MEDIUM PRIORITY - Slow message processing:"
...
if (( $(echo "$LOAD_AVG > 1.0" | bc -l) )); then
    echo "🟡 MEDIUM PRIORITY - System load elevated:"
